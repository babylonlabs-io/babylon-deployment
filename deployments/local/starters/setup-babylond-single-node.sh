...
if [[ "$IS_TGE" == 1 || "$IS_TGE" == "1" ]]; then
  ...
  | .app_state["btcstaking"]["params"][0]["btc_activation_height"]=2
...
perl -i -pe 's|timeout_commit = ".*?"|timeout_commit = "3s"|g' $n0cfg
...
perl -i -pe 's|network = "mainnet"|network = "regtest"|g' $n0app
